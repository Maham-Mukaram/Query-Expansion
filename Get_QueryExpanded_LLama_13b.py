...
TASK_LIST = ["scifact","trec-covid","webis-touche2020"]
...
for task in TASK_LIST:
    input_file = f"Datasets/Expanded_Llama_13b/{task}/queries.jsonl"
    output_dir = f"Datasets/Expanded_Llama_13b/{task}"
    ...
    for prompts in PROMPTS:
        ...
        with open(output_file, "a", encoding='utf8') as outfile:
            ...
            for line in loaded_data[len(all_responses):]:
                ...
                if prompt[1]=='CoT':
                    response=response[0]['generated_text'].split("Give the rationale before answering",1)
                    line["text"] = line.get("text")+" "+response[-1]
                else:
